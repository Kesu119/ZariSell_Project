...
import './lightGold2.css'
import { assets } from '../../assets/assets';

function LightGold2() {
    const images = [assets.light_shade2_2, assets.light_shade2_1,assets.light_shade2_3];
  const [mainImage, setMainImage] = useState(images[0]);
  const [liked, setLiked] = useState(false);
  const navigate = useNavigate(); 

  const handleOrderClick = () => {
    navigate("/order/Imitation Light gold shade2");
  };
  return (
    ...
    <div className="product-details">
      <h1>Imitation Light gold shade2</h1>
      <h3>Description:</h3>
